    ...
    function handleWeatherSearch(searchInput, searchOption) {
        ...
        if(searchOption === "citySearch") {
            ...
            fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${searchInput}&units=imperial&appid=${apiKey}`)
                .then(response => {
                    if(response.ok) {
                ...
                .then(response => {
                    ...
                    let newSearchInput = searchInput.split(" ").map(currentVal => {
            ...
            console.log("search lat = " + searchInput.lat);
            console.log("search lon = " + searchInput.lon);
