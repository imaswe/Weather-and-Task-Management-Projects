...
import styles from "./Weather.custom.css";
...
export default function Weather() {
    ...
    useEffect(() => {
        console.log("Main icon length: " + mainIcon.length);
    }, [mainIcon]);


    useEffect(() => {
        console.log("Lowest Degrees ----> " + lowestDeg);
    }, [lowestDeg]);


    useEffect(() => {
        console.log("Highest Degrees ----> " + highestDeg);
    }, [highestDeg]);
    ...
    useEffect(() => {
        if(todaysHourly.length > 0) {
            console.log("todays hourly = " + todaysHourly[0].times[0]);
        }
    }, [logger]);
    ...
    function handleTodaysWeather() {
        console.log("number of boxes = " + todaysWeather.length);
        let timeStamps = [];
    ...
    function handleTimeStamps(key) {
        setOccupied(true);
        let timeStamps = [];
        // console.log("Icon test ==== > " + numOfTimes[0].icons[0]);
        console.log("Temperature test ==== > " + numOfTimes[key].temps[0][3]);
        console.log("CLICKED !!! ----- > " + numOfTimes[key].counter);

        for(let i = 0; i < numOfTimes[key].counter; i++) {
            console.log("Icon Test =====> " + numOfTimes[key].icons[i]);
        ...
        // setTimeStampBoxes(prevStamps => {
        //     let newStamps = [...prevStamps, ...timeStamps];
        //     return newStamps;
        // });
    ...
    for(let i = 0; i < numOfDays; i++) {
        ...
        weatherBoxes.push( <div onClick={() => handleTimeStamps(i)} key={i} className="py-2 mb-5 flex flex-row items-center px-5 font-bold text-xl border-4 border-gray-300 cursor-pointer">
        <p>{daysOfTheWeek[dateUse.getDay()]}</p>
    ...
    if(todaysWeather.length > 0) {
        ...
        console.log("TEST LOW  ====> " + todaysLow);
        console.log("TEST HIGH ====> " + todaysHigh);
    }
    ...
    return (
        <>
            ...
            <h1 onMouseEnter={() => setIconGlow(true)} onMouseLeave={() => setIconGlow(false)} className="py-5 shadow-cyan-500 shadow-lg bg-gradient-to-r from-blue-400 to-cyan-300 rounded-full mt-5 text-4xl text-center text-blue-500 text-shadow-black">
                Weather Forecast 
            ...
            <div className="grid grid-cols-3 mt-10">
                ...
                <div className="col-span-1">
                    ...
                    <div className="flex flex-col bg-green-400">
                        ...
                        <label className="font-bold text-xl border-dashed border-4 border-white p-4 my-3" for="geoLat">Search By Geographic Coordinates <br /> <span className="text-red-500">&#10071;Please input POSITIVE values ONLY&#10071;</span> <br /> Maximum Latitude: 90 <br /> Maximum Longitude: 180</label>
                        ...
                        <input id="ZipSearch" className="text-xl border-4 border-green-900 p-4 my-3" placeholder="Input Zip Code" />
                        <button className="border-4 border-black bg-gray-400 py-3 hover:bg-gray-200 duration-300 font-bold text-xl">Search Weather</button>

                    </div>

                </div>
                {/* Right Container -- Weather Data -- takes up 2/3 of the width */}
                <main className="bg-blue-400 col-span-2 px-5 flex flex-col">
                    ...
                    <div className="flex flex-row border-8 border-blue-700 rounded-lg h-[1300px]">

                        {/* Left Container -- 1/3 width */}
                        <div className="bg-red-400 w-1/3 p-4 space-y-4">
                            ...
                            <p className="text-white font-bold text-2xl text-shadow-black">{loading === false && searchStatus === true? (
                                <>
                                    ...
                                    {/* <span className="text-black text-shadow-white"> {data.time} </span> */}
                                    <br />
                        ...
                        <div className="bg-green-500 w-2/3">
                            ...
                            <div className="flex flex-row space-x-10 px-5 py-5 border-blue-300 border-4 overflow-auto">
                            ...
                            {/* Line separator */}
                            <hr className="border-8 border-cyan-400 shadow-[0px_0px_5px_0px] shadow-cyan-400" />
                            
                            {/* X-Day Weather Forecast */}
                            <div className="flex flex-col text-white my-5">

                                {todaysWeather.length > 0 ? 
                                    <div onClick={handleTodaysWeather} className="py-2 mb-5 flex flex-row items-center px-5 font-bold text-xl border-4 border-gray-300 cursor-pointer">
                                        <p>Today</p>
                                        <img className="w-12 mx-14 scale-125" src={weatherMap.get(todaysWeather[0].icon)} />
                                        <p>{todaysWeather[0].low}&deg;</p>
                                        <div className={`mx-5 w-60 h-5 rounded-full bg-gradient-to-r ${todaysLow} ${todaysHigh}`}></div>
                                        <p>{todaysWeather[0].high}&deg;</p>
                                    </div> : ""}
